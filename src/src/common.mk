...
#uos os version
ifeq (${UOS_KVER},)
   UOS_KVER := $(shell echo ${KVER}| cut -d'-' -f2 | cut -d'.' -f1)
   EXTRA_CFLAGS += -DUOS_KERNEL_REVISON=${UOS_KVER}
endif

#euler os version
ifeq (${EULER_KVER},)
   EULER_KVER := $(shell echo ${KVER}| cut -d'-' -f2 | cut -d'.' -f1)
   EXTRA_CFLAGS += -DEULER_KERNEL_REVISON=${EULER_KVER}
endif
