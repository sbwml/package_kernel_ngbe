...
#endif
{
	...
	if (adapter->tx_itr_adapt) {
		ec->use_adaptive_tx_coalesce = 1;
	}
	if (adapter->rx_itr_adapt) {
...
#endif
{
	...
	if (ec->use_adaptive_tx_coalesce) {
		e_info(probe, "adaptive Tx itr turn ON\n");
		adapter->tx_itr_adapt = TRUE;
	} else {
		e_info(probe, "adaptive Tx itr turn OFF\n");
		adapter->tx_itr_adapt = FALSE;
	}

	if (ec->use_adaptive_rx_coalesce) {
		e_info(probe, "adaptive Rx itr turn ON\n");
		adapter->rx_itr_adapt = TRUE;
	} else {
		e_info(probe, "adaptive Rx itr turn OFF\n");
		adapter->rx_itr_adapt = FALSE;
	}
